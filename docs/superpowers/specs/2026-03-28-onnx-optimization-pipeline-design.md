...
The build container installs `onnx`, `onnxruntime==1.26.0`, and CPU-only `torch`. `torch` is required only at build time so `onnxruntime.transformers.optimizer.optimize_model()` can run for BERT-family models; the final runtime artifact does not depend on PyTorch.
